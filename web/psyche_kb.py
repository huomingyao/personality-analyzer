...
@app.route('/api/kb/debug-load', methods=['GET'])
def api_kb_debug_load():
    """DEBUG: 加载知识库材料并返回前500字符"""
    try:
        kb_name = request.args.get('name', '').strip()
        kb_filename = request.args.get('file', '').strip()

        from analyze_service import AnalyzeService
        svc = AnalyzeService()
        svc.set_knowledge_base(kb_name)

        if kb_filename:
            content = svc.browse_document(kb_filename)
        else:
            doc_names = svc.browser.list_documents()
            all_content = []
            for doc_id in doc_names:
                doc = svc.browser.read_document(doc_id)
                if doc:
...
def api_kb_delete():
    ...
    try:
        ...
        if not name:
            return jsonify({"success": False, "message": "请提供档案库名称"})

        kb_dir = os.path.join(os.path.dirname(__file__), "knowledge_bases", name)
        if not os.path.exists(kb_dir):
            return jsonify({"success": False, "message": "档案库不存在"})

        shutil.rmtree(kb_dir)
        return jsonify({"success": True, "message": f"已删除档案库「{name}」"})
    except Exception as e:
        return jsonify({"success": False, "message": str(e)})
...
def api_analyze():
    """心理分析 - 支持四种模式:
    1. 直接提供 materials
    2. 通过 kb_name + kb_filename 指定知识库文件
    3. 通过 kb_name + kb_search 让 Agent 主动搜索知识库
    4. 仅提供 kb_name 时自动加载知识库全部文档
    """
    try:
        data = request.get_json()
        ...
        kb_filename = data.get('kb_filename', '').strip()
...
def api_analyze_stream():
    ...
    kb_search = data.get('kb_search', '').strip()
    kb_filename = data.get('kb_filename', '').strip()
    enable_critic = data.get('critic', True)
    ...
    init_event = {"type": "init", "target": target}
    if kb_name:
        init_event["kb_name"] = kb_name
    progress_queue.put(init_event)
    ...
    def run_analysis():
        try:
            service = get_service()
            if enable_critic:
                on_progress("Critic 审查已启用，分析后将自动校验")
            result = service.analyze(
                target, materials, framework or None,
                kb_name=kb_name,
                kb_search=kb_search,
